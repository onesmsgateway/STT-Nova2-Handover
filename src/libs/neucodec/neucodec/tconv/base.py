...
class FirstBlock(nn.Module):
    ...
    def __init__(self, target_dim, conv_kernels=(7, 7, 7, 7), pool_kernels=(1, 3, 5, 9), dilation_rate=2):
        ...
        for conv_kernel, pool_kernel in zip(conv_kernels, pool_kernels):
            ...
            blocks.append(nn.Sequential(TrendPool(pool_kernel), Conv1d(1, each_dim, kernel_size=conv_kernel, dilation=conv_dilation, padding=conv_padding)))
...
class EnhanceBlock(FirstBlock):
    ...
    def __init__(self, dim):
        ...
        self.dim = dim
        self.merge_layer = nn.Sequential(nn.InstanceNorm1d(4, affine=True), nn.Conv1d(4, 1, kernel_size=1))
...
class SimpleEnhanceBlock(FirstBlock):
    ...
    def __init__(self, dim):
        super().__init__(4, conv_kernels=(7, 7, 7, 7), pool_kernels=(1, 3, 5, 9))
        self.dim = dim
        self.merge_layer = nn.Sequential(nn.InstanceNorm1d(4, affine=True), nn.Conv1d(4, self.dim, kernel_size=1))
