...
class NeuCodec(nn.Module, PyTorchModelHubMixin, repo_url='https://github.com/neuphonic/neucodec', license='apache-2.0'):
    ...
    @classmethod
    def _from_pretrained(cls, *, model_id: str, revision: Optional[str]=None, cache_dir: Optional[str]=None, force_download: bool=False, proxies: Optional[Dict]=None, resume_download: bool=False, local_files_only: bool=False, token: Optional[str]=None, map_location: str='cpu', strict: bool=True, **model_kwargs):
        ...
        if model_id == 'neuphonic/neucodec':
            ...
            ignore_keys = []
        ckpt_path = hf_hub_download(repo_id=model_id, filename='pytorch_model.bin', revision=revision, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download, local_files_only=local_files_only, token=token)
        _ = hf_hub_download(repo_id=model_id, filename='meta.yaml', revision=revision, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download, local_files_only=local_files_only, token=token)
        model = cls(24000, 480)
    ...
    def _prepare_audio(self, audio_or_path: torch.Tensor | Path | str):
        if isinstance(audio_or_path, (Path, str)):
            ...
            if sr != 16000:
                y, sr = (T.Resample(sr, 16000)(y), 16000)
                y = y[None, :]
            ...
            if len(y.shape) == 3:
                ...
                raise ValueError(f'NeuCodec expects tensor audio input to be of shape [B, 1, T] -- received shape: {y.shape}')
    ...
    def encode_code(self, audio_or_path: torch.Tensor | Path | str) -> torch.Tensor:
        ...
        semantic_features = self.feature_extractor(y.squeeze(0), sampling_rate=16000, return_tensors='pt').input_features.to(self.device)
...
class DistillNeuCodec(NeuCodec):
    ...
    def encode_code(self, audio_or_path: torch.Tensor | Path | str) -> torch.Tensor:
        y = self._prepare_audio(audio_or_path)
        semantic_features = self.feature_extractor(F.pad(y[0, :].cpu(), (160, 160)), sampling_rate=16000, return_tensors='pt').input_values.to(self.device).squeeze(0)
...
class NeuCodecOnnxDecoder(ModelHubMixin, repo_url='https://github.com/neuphonic/neucodec', license='apache-2.0'):
    ...
    @classmethod
    def _from_pretrained(cls, *, model_id: str, revision: Optional[str]=None, cache_dir: Optional[str]=None, force_download: bool=False, proxies: Optional[Dict]=None, resume_download: bool=False, local_files_only: bool=False, token: Optional[str]=None, map_location: str='cpu', strict: bool=True, **model_kwargs):
        onnx_path = hf_hub_download(repo_id=model_id, filename='model.onnx', revision=revision, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download, local_files_only=local_files_only, token=token)
        _ = hf_hub_download(repo_id=model_id, filename='meta.yaml', revision=revision, cache_dir=cache_dir, force_download=force_download, proxies=proxies, resume_download=resume_download, local_files_only=local_files_only, token=token)
        model = cls(onnx_path)
        if map_location != 'cpu':
    ...
    def encode_code(self, *args, **kwargs):
        raise NotImplementedError('The onnx decoder has no functionality to encode codes, as it only contains the compiled decoder graph.')
